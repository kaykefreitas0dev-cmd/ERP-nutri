...
export function SignupForm({ plan }: { plan: string }) {
  ...
  async function handleSubmit(formData: FormData) {
    ...
    startTransition(async () => {
      const result = await submitSignupAction(formData);
      setState({
  ...
  return (
    <form action={handleSubmit} className="space-y-4" noValidate>
      ...
      {planLabel && (
        <div className="rounded-md border border-brand-primary/30 bg-brand-primary-bg px-4 py-3 text-sm text-text-secondary">
          Plano escolhido:{" "}
          <span className="font-semibold text-brand-primary">{planLabel}</span>{" "}
      ...
      <div>
        ...
        <textarea
          ...
          name="note"
          rows={3}
          placeholder="Ex: quantos pacientes você atende, se tem equipe…"
          className="mt-1 block w-full rounded-md border border-border-default bg-white px-3 py-2 text-sm shadow-sm focus:border-brand-primary focus:outline-none focus:ring-1 focus:ring-brand-primary"
        />
      </div>
      ...
      <button
        ...
        className="inline-flex h-11 w-full items-center justify-center rounded-md bg-brand-primary px-6 text-sm font-medium text-white hover:bg-brand-primary-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
      >
