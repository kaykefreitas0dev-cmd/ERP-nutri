...
import {
  listPatientNotificationsAction,
  getPatientUnreadCountAction,
...
/** Sino de notificações do paciente (topbar do PWA). */
export function NotificationBell() {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [unread, setUnread] = useState(0);
  const [items, setItems] = useState<PatientNotificationItem[]>([]);
  ...
  useEffect(() => {
    ...
    async function refresh() {
      const count = await getPatientUnreadCountAction();
  ...
  const loadList = useCallback(async () => {
    setLoading(true);
    try {
      const result = await listPatientNotificationsAction(20);
  ...
  function handleItemClick(n: PatientNotificationItem) {
    if (!n.read) {
      setItems((prev) =>
      ...
      startTransition(async () => {
        await markPatientNotificationReadAction(n.id);
      });
    }
    if (n.linkPath) {
      setOpen(false);
  ...
  function handleMarkAll() {
    ...
    setUnread(0);
    startTransition(async () => {
      await markAllPatientNotificationsReadAction();
    });
  }
  ...
  return (
    <div ref={rootRef} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full z-[90] mt-2 w-80 max-w-[calc(100vw-2rem)] overflow-hidden rounded-lg border border-border-subtle bg-bg-surface [box-shadow:var(--shadow-lg)]">
